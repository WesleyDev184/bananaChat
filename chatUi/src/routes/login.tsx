...
function LoginPage() {
  ...
  const { username, setUsername, autoConnect, redirectTo } = useLoginParams();

  const { connectionStatus, isLoadingHistory, isConnected } =
  ...
  const handleJoinChat = useCallback(() => {
    ...
    // Redirecionar para o chat com o username como parâmetro
    navigate({
      to: redirectTo,
      search: { username, autoJoin: true },
    });
  }, [username, isConnected, navigate, redirectTo]);

  // Auto-join se autoConnect estiver ativado
  useEffect(() => {
    if (username && autoConnect && isConnected) {
      handleJoinChat();
    }
  }, [username, autoConnect, isConnected, handleJoinChat]);
